 ...
 */
class BaseProductSelectEventBus {
    // MutableSharedFlow Param replay: #  of values replayed to new subscribers (cannot be negative, defaults to zero).
    private val _events = MutableSharedFlow<BaseProductSelectEvent>(replay = 0)

    val events = _events.asSharedFlow() // publicly exposed as read-only shared flow

    suspend fun produceEvent(event: BaseProductSelectEvent) {
